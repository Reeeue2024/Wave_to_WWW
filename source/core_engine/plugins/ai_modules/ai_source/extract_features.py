...
def extract_features(url):
    ...
    return {
        ...
        'value_entropy_url': round(shannon_entropy(full_url), 2),
        'ratio_alpha_numeric': round(
            sum(c.isalpha() for c in full_url) / (sum(c.isalnum() for c in full_url) + 1e-5), 4),
        'domain_complexity': len(domain) + round(shannon_entropy(domain), 2),
        ...
        'path_entropy': round(shannon_entropy(path), 2),
