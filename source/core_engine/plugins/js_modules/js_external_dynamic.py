# [ Core ] Module - JS : js_external_dynamic.py ( Score )
...
class JsExternalDynamic(BaseModule) :
    ...
    def scan(self) :
        ...
        js_file_path = os.path.abspath(os.path.join(js_directory_path, "js_external_dynamic.js"))
        ...
        try :
            ...
            if result_object.get("score", 0) >= 50 :
                ...
                self.module_result_data["reason"] = "External Dynamic Score : High."

            elif result_object.get("score", 0) >= 20 :

                self.module_result_flag = True
                self.module_result_data["reason"] = "External Dynamic Score : Not High / Not Low."

            else :

                self.module_result_flag = False
                self.module_result_data["reason"] = "External Dynamic Score : Low."
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) != 2 :
        ...
        print("How to Use : python3 js_exfil_dynamic.py < URL >")

        sys.exit(1)

    input_url = sys.argv[1]

    module_instance = JsExternalDynamic(input_url)
