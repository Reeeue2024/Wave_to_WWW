# [ Core ] Module - JS : js_script_dynamic.py ( Score )
...
class JsScriptDynamic(BaseModule) :
    ...
    def scan(self) :
        ...
        js_file_path = os.path.abspath(os.path.join(js_directory_path, "js_script_dynamic.js"))
        ...
        try :
            ...
            if result_object.get("score", 0) >= 50 :
                ...
                self.module_result_data["reason"] = "Script Dynamic Score : High."
            
            elif result_object.get("score", 0) >= 20 :

                self.module_result_flag = True
                self.module_result_data["reason"] = "Script Dynamic Score : Not High / Not Low."
            
            else :
                
                self.module_result_flag = False
                self.module_result_data["reason"] = "Script Dynamic Score : Low."
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) != 2 :
        ...
        print("How to Use : python3 js_script_dynamic.py < URL >")

        sys.exit(1)

    input_url = sys.argv[1]

    module_instance = JsScriptDynamic(input_url)
