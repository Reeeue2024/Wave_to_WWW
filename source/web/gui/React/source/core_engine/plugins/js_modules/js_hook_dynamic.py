# [ Core ] Module - JS : js_hook_dynamic.py ( Score )
...
class JsHookDynamic(BaseModule) :
    ...
    def scan(self) :
        ...
        js_file_path = os.path.abspath(os.path.join(js_directory_path, "js_hook_dynamic.js"))
        ...
        try :
            ...
            if result_object.get("score", 0) >= 50 :
                ...
                self.module_result_data["reason"] = "Hook Dynamic Score : High."
            
            elif result_object.get("score", 0) >= 20 :

                self.module_result_flag = True
                self.module_result_data["reason"] = "Hook Dynamic Score : Not High / Not Low."
            
            else :

                self.module_result_flag = False
                self.module_result_data["reason"] = "Hook Dynamic Score : Low."
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) != 2 :
        ...
        print("How to Use : python3 js_hook_dynamic.py < URL >")

        sys.exit(1)

    input_url = sys.argv[1]

    module_instance = JsHookDynamic(input_url)
