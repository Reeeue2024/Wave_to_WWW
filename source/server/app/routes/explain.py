...
def risk_policy(score: float) -> Dict[str, str]:
...
def interpretation_ko(band: str) -> str:
...
def interpretation_en(band: str) -> str:
...
def extract_ai_score(modules: List[ModuleItem]) -> Optional[float]:
    for m in modules:
...
def friendly_reason(module: str, reason: Any, lang: str) -> str:
...
def prompt_ko(payload: Dict, pol: Dict) -> str:
    ...
    interpretation_base = interpretation_ko(pol['band'])
...
5. **KISA 제보 문구는 제공된 경우에만 반드시 그대로, 별도의 줄에 출력하세요.**

다음 구조로 정확히 출력하세요:
전체 판단: {pol['verdict_ko']} ({score_overall}%)
탐지 현황: {flagged}/{total}개 항목에서 위험 신호 발견
AI 위험도: {ai_str} (AI 분석 기준으로 피싱일 확률을 수치화한 값)
...
def prompt_en(payload: Dict, pol: Dict) -> str:
    score_overall = int(round(payload["overall"]["resultScore"]))
    total = payload["counts"]["total"]
    flagged = payload["counts"]["flagged"]
    ai_score = payload.get("aiScore", None)
    ...
    interpretation_base = interpretation_en(pol['band'])
...
Detection status: Found risk signals in {flagged} of {total} items 
AI risk score: {ai_str} (probability estimate from AI analysis)
...
def explain(req: ExplainRequest):
    # 1) AI 점수
    ai_score = extract_ai_score(req.modules)
    ...
    for x in detected_list:
        lang = req.lang
        x["friendly"] = friendly_reason(x["name_raw"], x["reason_raw"], lang)
    ...
    payload = {
        ...
        "aiScore": ai_score,
        "reportedToKisa": bool(req.summary.resultFlag or req.summary.resultScore >= 70),
    }

    pol = risk_policy(req.summary.resultScore)

    # 6) OpenAI 미사용 시 폴백 출력
    if client is None:
        score_overall = int(round(req.summary.resultScore))
        ai_str = f"{int(round(ai_score))}%" if isinstance(ai_score, (int, float)) else "N/A"
        reported_to_kisa = bool(req.summary.resultFlag or req.summary.resultScore >= 70)
        ...
        if req.lang == "ko":
            ...
            header = (
                ...
                f"Modules flagged: {flagged} of {total}. \nAI score: {ai_str}.\n"
    ...
    user_prompt = (prompt_ko if req.lang == "ko" else prompt_en)(payload, pol)
